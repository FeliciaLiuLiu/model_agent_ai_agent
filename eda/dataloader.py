...
def _dtype_family_pandas(series: pd.Series) -> str:
...
def _candidate_key_columns(df: pd.DataFrame) -> List[str]:
    candidates: List[str] = []
    for col in df.columns:
        series = df[col]
        family = _dtype_family_pandas(series)
        if family in {"datetime", "other"}:
            continue
        unique_ratio = _series_unique_ratio(series)
        if _is_id_like_name(col) or unique_ratio >= 0.4:
            candidates.append(col)
    if not candidates:
        for col in df.columns:
            family = _dtype_family_pandas(df[col])
            if family in {"numeric", "string", "bool"}:
                candidates.append(col)
    return candidates[:40]


def _infer_join_mapping_pandas(
    ...
    left_candidates = _candidate_key_columns(left_df)
    right_candidates = _candidate_key_columns(right_df)
    ...
    for left_col in left_candidates:
        ...
        left_type = _dtype_family_pandas(left_series)
        ...
        for right_col in right_candidates:
            ...
            right_type = _dtype_family_pandas(right_series)
...
def _infer_base_table(tables: Dict[str, pd.DataFrame]) -> str:
    preferred = ("transaction", "transactions", "txn", "fact_transaction", "fact")
    for name in preferred:
        if name in tables:
            return name
    return max(tables.items(), key=lambda x: int(x[1].shape[0]))[0]
...
def _prepare_right_for_merge(
    ...
    for col in right_df.columns:
        if col in right_on:
            ...
            rename_map[col] = f"{table_name}__{col}"
    prepared = right_df.rename(columns=rename_map)
    return prepared, renamed_keys


def _aggregate_tables_pandas(tables: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    rows: List[Dict[str, Any]] = []
    for name, df in tables.items():
        numeric_cols = int(df.select_dtypes(include=["number", "bool"]).shape[1])
        rows.append(
            {
                "table_name": name,
                ...
                "non_numeric_column_count": int(df.shape[1] - numeric_cols),
...
def _evaluate_consistency_checks(
    df: pd.DataFrame,
    ...
    for check in checks:
        ...
        if left_col not in df.columns or right_col not in df.columns:
            ...
            continue
        both = df[left_col].notna() & df[right_col].notna()
        ...
        if comparable == 0:
            ...
            continue
        mismatch = int(((df[left_col] != df[right_col]) & both).sum())
        reports.append(
            {
                "name": name,
                "left": left_col,
...
def _compose_tables_pandas(
    ...
    if raw_joins is None:
        for table_name in tables.keys():
            if table_name == base:
                continue
            inferred = _infer_join_mapping_pandas(
                left_df=tables[base],
    ...
    if not joins or unjoinable:
        if policy == "aggregate_only":
            return _aggregate_tables_pandas(tables), {
                "mode": "aggregate_only",
    ...
    for join in joins:
        ...
        prepared_right, renamed_keys = _prepare_right_for_merge(right_df, right_on, right_name)
        marker = f"__joined__{right_name}"
        prepared_right[marker] = 1
        merge_kwargs: Dict[str, Any] = {
            "how": join["how"],
            "left_on": left_on,
            "right_on": renamed_keys,
            "sort": False,
        }
        validate = join.get("validate")
        if validate:
            merge_kwargs["validate"] = validate

        try:
            merged = composed.merge(prepared_right, **merge_kwargs)
        ...
        unmatched_rows = int(merged[marker].isna().sum())
        composed = merged.drop(columns=[marker] + renamed_keys, errors="ignore")
        join_reports.append(
            {
                ...
                "left_on": left_on,
                "right_on": right_on,
                "rows_after_join": int(composed.shape[0]),
    ...
    consistency = _evaluate_consistency_checks(composed, checks) if checks else []
    meta = {
        "mode": "row_level",
        "base_table": base,
        "tables": list(tables.keys()),
...
def _ensure_table_dict(obj: Any, source_name: str) -> Dict[str, pd.DataFrame]:
    ...
    for key, value in obj.items():
        ...
        if not isinstance(value, pd.DataFrame):
            raise ValueError(f"{source_name} table '{name}' is not a pandas DataFrame.")
        tables[name] = value
    if not tables:
...
def _merge_table_frame(tables: Dict[str, pd.DataFrame], table_name: str, frame: pd.DataFrame) -> None:
...
def _merge_loaded_object_into_tables(
    ...
    if isinstance(obj, Mapping):
        table_dict = _ensure_table_dict(obj, source_name)
        for table_name, frame in table_dict.items():
            _merge_table_frame(tables, _normalize_table_name(table_name), frame)
        return
    if not isinstance(obj, pd.DataFrame):
        raise ValueError(f"{source_name} did not return a pandas DataFrame.")
    _merge_table_frame(tables, _normalize_table_name(default_table_name), obj)


def _load_sql_auto(sql_files: List[Path], db_files: List[Path]) -> List[Tuple[str, pd.DataFrame]]:
    if not sql_files:
        return []
    ...
    if db_files and all(_is_select_only(text) for _, text in texts):
        ...
        conn = sqlite3.connect(db_path)
        try:
            frames = [(_table_name_from_path(path), pd.read_sql_query(text, conn)) for path, text in texts]
        finally:
            conn.close()
        return frames
    ...
    try:
        ...
        return [(_normalize_table_name(table), frame)]
    finally:
        conn.close()
...
class DataLoader:
    ...
    def load(self) -> Tuple[pd.DataFrame, str]:
        ...
        if mode == "sql":
            ...
            if named_sql:
                tables = {name: _load_sql(query, self.db) for name, query in named_sql.items()}
                df, meta = _compose_tables_pandas(tables, compose_spec=compose_spec, no_key_policy=self.no_key_policy)
        ...
        if mode == "py":
            ...
            if isinstance(obj, Mapping):
                tables = _ensure_table_dict(obj, "Python loader")
                df, meta = _compose_tables_pandas(tables, compose_spec=compose_spec, no_key_policy=self.no_key_policy)
                self.last_compose_meta = meta
                return df, f"py:{self.py}"
            if not isinstance(obj, pd.DataFrame):
                raise ValueError("Python loader did not return a pandas DataFrame.")
            return obj, f"py:{self.py}"

        if mode == "py_code":
            obj = _load_python_code_object(self.py_code or "")
            if isinstance(obj, Mapping):
                tables = _ensure_table_dict(obj, "Python code")
                df, meta = _compose_tables_pandas(tables, compose_spec=compose_spec, no_key_policy=self.no_key_policy)
                self.last_compose_meta = meta
                return df, "py_code"
            if not isinstance(obj, pd.DataFrame):
        ...
        if mode == "nb":
            ...
            if isinstance(obj, Mapping):
                tables = _ensure_table_dict(obj, "Notebook loader")
                df, meta = _compose_tables_pandas(tables, compose_spec=compose_spec, no_key_policy=self.no_key_policy)
                self.last_compose_meta = meta
                return df, f"nb:{self.nb}"
            if not isinstance(obj, pd.DataFrame):
                raise ValueError("Notebook execution did not return a pandas DataFrame.")
            return obj, f"nb:{self.nb}"
        ...
        if mode == "auto":
            ...
            if data_paths:
                ...
                for p in data_paths:
                    ...
                    grouped_frames.setdefault(table_name, []).append(_read_single(p))
            ...
            if sql_files:
                sql_tables = _load_sql_auto(sql_files, db_files)
    ...
    def _load_tables_for_data_mode(
        ...
        for path in paths:
            ...
            grouped_frames.setdefault(table_name, []).append(_read_single(path))
